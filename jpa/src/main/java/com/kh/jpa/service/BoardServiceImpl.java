...
//                boardTag.changeBoard(board);
    ...
    @Transactional
    public UpdateDto.Response update(Long boardId, UpdateDto.Response updateDto) {

        // 1. 게시글 조회
        Board board = boardRepository.findById(boardId)
                .orElseThrow(() -> new IllegalArgumentException("게시글이 존재하지 않습니다."));

        // 2. PATCH - null 아닌 것만 수정
        if (updateDto.getBoardTitle() != null) {
            board.updateTitle(updateDto.getBoardTitle());
        }

        if (updateDto.getBoardContent() != null) {
            board.updateContent(updateDto.getBoardContent());
        }

        if (updateDto.getOriginName() != null) {
            board.updateName(updateDto.getOriginName());
        }


        // 3. save() ❌ 필요 없음 (Dirty Checking)
        // boardRepository.save(board);

        // 4. 응답 DTO 변환
        return UpdateDto.Response.from(board);
    }
